...
class AntEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def _set_state(self, state):
        qpos = state[1:16]
        qvel = state[16:]
        self.xposbefore = -1*(state[0] * self.dt - qpos[0])
    ...
    def torch_reward_fn(self):
        def _thunk(obs, act, next_obs):
            reward_ctrl = -0.005 * torch.sum(torch.square(act), axis=-1)
            reward_run = obs[..., 0]
    ...
    def torch_done_fn(self):
        def _thunk(next_obs):
            notdone =  torch.logical_and(next_obs[:,1] >= 0.2 , next_obs[:,1] <= 1.0) #notdone =  obs[:,1] >= 0.2 and obs[:,1] <= 1.0
            return torch.logical_not(notdone)
        return _thunk
